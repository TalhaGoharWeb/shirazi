...
class HudCanvas(QWidget):
    def __init__(self, face_path: str, assistant_name: str = "SHIRAZI", parent=None):
    ...
    def _step(self):
        ...
        # a rate that changes with state jumps the rings the instant SHIRAZI
    ...
    def _state_display(self) -> str:
        ...
        # SHIRAZI's own voice while speaking). Falls back to a gentle idle
...
class LogWidget(QTextEdit):
    ...
    def __init__(self, parent=None):
        ...
        self._pos     = 0
        self._tag     = "sys"
        self._ai_name_lc = "shirazi"   # updated when assistant name changes
        self._tmr = QTimer(self)
        self._tmr.timeout.connect(self._step)
    ...
    def _next(self):
        ...
        elif tl.startswith(_ai_pfx) or tl.startswith(("shirazi:", "jarvis:")): self._tag = "ai"
...
class FileDropZone(QWidget):
    ...
    def _browse(self):
        path, _ = QFileDialog.getOpenFileName(
            self, "Select a file for SHIRAZI", str(Path.home()),
            "All Files (*.*);;"
...
class SetupOverlay(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        layout.addWidget(_lbl("Configure SHIRAZI before first boot.", 9, color=C.PRI_DIM))
        layout.addSpacing(6)
...
class CustomizeOverlay(QWidget):
    ...
    def __init__(self, assistant_name="SHIRAZI", user_name="",
    ...
    def _save(self):
        name = self._name_input.text().strip() or "SHIRAZI"
...
class MemoryOverlay(_HudOverlay):
    """Everything SHIRAZI has stored about you, and when it learned it.
    ...
    def _rebuild(self):
        ...
        hdr = QLabel("🧠  WHAT SHIRAZI REMEMBERS")
        hdr.setFont(QFont("Courier New", 12, QFont.Weight.Bold))
        hdr.setStyleSheet(f"color: {C.PRI}; background: transparent;")
...
class ClipboardPanel(QWidget):
    """Floating panel shown when text is copied — offers quick Shirazi actions."""
...
class RemoteKeyOverlay(QWidget):
    ...
    def mark_connected(self) -> None:
        ...
        self._timer_lbl.setText("Phone connected — SHIRAZI ready")
...
class MainWindow(QMainWindow):
    ...
    def __init__(self, face_path: str):
        ...
        self._assistant_name: str = (_cfg.get("assistant_name") or "SHIRAZI").strip()
        ...
        self.on_interrupt      = None   # callable: () -> None — stop SHIRAZI mid-speech
        self.on_voice_change   = None   # callable: () -> None — rebuild session with new voice
        self.on_audio_device_change = None  # callable: () -> None — reopen audio streams
        self._confirm_overlay  = None   # live ConfirmBanner, if one is on screen
        self.get_plugins       = None   # callable: () -> list[dict], set by ShiraziLive
        self.get_plugin_settings = None # callable: () -> list[dict] settings schemas, set by ShiraziLive
        self.on_wake_toggle    = None   # callable: (enable: bool) -> str, set by ShiraziLive
    ...
    # ------------------------------------------------------------------
    @staticmethod
    def _build_shirazi_icon(out_path: Path) -> bool:
        """
        Render a SHIRAZI arc-reactor icon at 4× resolution and downsample
    ...
    def _create_lnk_windows(lnk: str, target: str, args: str,
        ...
        try:
            ...
            sc.Description      = "SHIRAZI AI Assistant"
    ...
    def _create_desktop_shortcut(self):
        ...
        ico_path = Path(__file__).resolve().parent / "config" / "shirazi.ico"
        _legacy_ico = Path(__file__).resolve().parent / "config" / "jarvis.ico"
        if not ico_path.exists() and _legacy_ico.exists():
            ico_path = _legacy_ico  # pre-rebrand install; shirazi.ico ships from Phase 3
        if not ico_path.exists():
            self._build_shirazi_icon(ico_path)
        ...
        try:
            ...
            # ── macOS — proper .app bundle (no Terminal window) ───────────────
                ...
                (app / "Contents" / "Info.plist").write_text(
                    ...
                    '  <key>CFBundleExecutable</key><string>SHIRAZI</string>\n'
                    '  <key>CFBundleIdentifier</key>'
                    '<string>com.shirazi.assistant</string>\n'
                    '  <key>CFBundleName</key><string>SHIRAZI</string>\n'
    ...
    def _build_header(self) -> QWidget:
        ...
        _sub_text = ("A Friendly Assistant"
                     if _disp in ("SHIRAZI", "JARVIS", "J.A.R.V.I.S")  # + legacy Mark-LIV defaults
    ...
    # arrives the way everything else does: SHIRAZI says it out loud.
    ...
    # a dropped file is — as a message SHIRAZI reads and responds to. That keeps
    ...
    def _quiz_submit(self, given: str):
        ...
        if verdict is True:
            ...
            # Open answers and near-miss gap-fills are SHIRAZI's to judge. Saying
    ...
    def _quiz_finish(self):
        ...
        # Hand it back to SHIRAZI as a message, not as a tool return: the tool
    ...
    # Legacy Mark-LIV autostart ids, recognised (and cleaned up) on upgrade.
    _LEGACY_AUTOSTART_WIN = "JARVIS_AI"
    _LEGACY_AUTOSTART_MAC = "com.jarvis.assistant.plist"
    _LEGACY_AUTOSTART_LINUX = "jarvis.desktop"

    def _check_autostart(self) -> bool:
        """Returns True if auto-start is currently registered on this OS.

        Recognises both the Shirazi ids and the legacy Mark-LIV ids, so an
        upgraded install is not misreported as "off" while a legacy entry
        still autostarts the app.
        """
        try:
            if _OS == "Windows":
                import winreg
                ...
                try:
                    for _name in ("SHIRAZI_AI", self._LEGACY_AUTOSTART_WIN):
                        try:
                ...
                _d = Path.home() / "Library" / "LaunchAgents"
                return ((_d / "com.shirazi.assistant.plist").exists()
                        or (_d / self._LEGACY_AUTOSTART_MAC).exists())
            else:
                _d = Path.home() / ".config" / "autostart"
                return ((_d / "shirazi.desktop").exists()
                        or (_d / self._LEGACY_AUTOSTART_LINUX).exists())
        except Exception:
            return False
    ...
    def _toggle_autostart(self):
        ...
        try:
            ...
            if _OS == "Windows":
                ...
                # Remove both ids: a legacy Mark-LIV entry must not survive.
                for _name in ("SHIRAZI_AI", self._LEGACY_AUTOSTART_WIN):
                    try:
                        winreg.DeleteValue(reg, _name)
                    except FileNotFoundError:
                        pass
                if not currently_on:
                    pythonw = Path(sys.executable).parent / "pythonw.exe"
                    exe = str(pythonw if pythonw.exists() else sys.executable)
                    winreg.SetValueEx(reg, "SHIRAZI_AI", 0, winreg.REG_SZ,
                                      f'"{exe}" "{script}"')
                winreg.CloseKey(reg)
            elif _OS == "Darwin":
                plist_dir = Path.home() / "Library" / "LaunchAgents"
                plist_dir.mkdir(parents=True, exist_ok=True)
                plist = plist_dir / "com.shirazi.assistant.plist"
                (plist_dir / self._LEGACY_AUTOSTART_MAC).unlink(missing_ok=True)
                if currently_on:
                    ...
                    plist.write_text(
                        ...
                        '"http://www.apple.com/DTDs/PropertyList-1.0.dtd">\n'
                        '<plist version="1.0"><dict>\n'
                        '  <key>Label</key><string>com.shirazi.assistant</string>\n'
                ...
                desk = desk_dir / "shirazi.desktop"
                (desk_dir / self._LEGACY_AUTOSTART_LINUX).unlink(missing_ok=True)
                if currently_on:
    ...
    def _wake_state(self) -> dict:
        ...
        if self.wake_get_state:
            try:
                ...
                pass
        # Before ShiraziLive has wired its callback (drawer built at startup).
    ...
    def _refresh_wake_btns(self):
        if not hasattr(self, '_wake_btn'):
            return
        st = self._wake_state()
        # Honest wake-phrase tooltip: the configured (branded) phrases plus
        # the phrase the legacy model actually listens for today.
        try:
            from core.wake_word import effective_listening_phrase
            from memory.config_manager import get_wake_words
            _words = get_wake_words()
            _pretty = ", ".join(w.replace("_", " ").title() for w in _words)
            self._wake_btn.setToolTip(
                "Wake phrases: " + _pretty + ". "
                + "Detector listens for '" + effective_listening_phrase() + "' "
                + "(Hey-Shirazi model pending; see docs/LEGACY_COMPAT.md).")
        except Exception:
            pass
        _on = f"""
    ...
    def _toggle_wake_word(self):
        ...
        if not st["ready"]:
            ...
            return
        # Already downloaded → just flip enabled/disabled through ShiraziLive.
    ...
    def _open_customize(self):
        ...
        cw = self.centralWidget()
        ov = CustomizeOverlay(
            cfg.get("assistant_name", "SHIRAZI") or "SHIRAZI",
            cfg.get("user_name", ""),
    ...
    def _apply_name_update(self, name: str, user_name: str, ui_color: str = "",
        ...
        self._assistant_name = name.strip() or "SHIRAZI"
        display = self._assistant_name.upper()
        self.setWindowTitle(f"{display} — {APP_VERSION}")
        self._title_lbl.setText(display)
        if display in ("JARVIS", "J.A.R.V.I.S"):  # legacy Mark-LIV names keep their backronym
    ...
    def _on_setup_done(self, key: str, os_name: str):
        ...
        if self._overlay:
            ...
            self._overlay = None
        self._apply_state("LISTENING")
        self._assistant_name = _read_full_config().get("assistant_name", "SHIRAZI") or "SHIRAZI"
...
class ShiraziUI:
    ...
    def push_visemes(self, frames, hop: float, at: float) -> None:
        ...
        for SHIRAZI's own speech. `at` is the wall-clock time the batch begins to
    ...
    def show_quiz(self, topic: str, questions, grade=None) -> None:
        ...
        here. Returning None from it means "SHIRAZI should judge this one", which
...
# Legacy alias: user scripts written for the Mark-LIV era may `from ui import
# JarvisUI`. The alias is permanent; the class itself is ShiraziUI.
JarvisUI = ShiraziUI
